 ...
 class RatingControllerTest {
    ...
    void testGetRatingsByProductId() throws Exception {
        ...
        mockMvc.perform(get("/rating/product/{productId}", productId))
                ...
                .andExpect(jsonPath("$[0].customerName").value("JohnDoe"));
    }
    ...
    void testGetAllRatings_NoRatingsFound() throws Exception {
        ...
        mockMvc.perform(get("/rating"))
                .andExpect(status().isInternalServerError())
                .andExpect(content().string("No ratings found"));
    }
    ...
    void testAddRating_Exception() throws Exception {
        ...
        when(modelMapper.map(ratingDTO, Rating.class)).thenReturn(rating);
        when(orderService.getOrderById(1)).thenReturn(order);
        when(ratingService.addRating(any(Rating.class))).thenThrow(new RatingException("Failed to add rating"));
        ...
        mockMvc.perform(post("/rating")
                ...
                .content(objectMapper.writeValueAsString(ratingDTO)))
                .andExpect(status().isInternalServerError())
                .andExpect(content().string("Failed to add rating"));
    }
    
    @Test
    void testGetRatingsByProductId_NoRatingsFound() throws Exception {
        // Arrange
        Integer productId = 1;
        when(ratingService.getRatingsByProductId(productId)).thenThrow(new RatingException("No ratings found for product"));
        
        // Act & Assert
        mockMvc.perform(get("/rating/product/{productId}", productId))
                .andExpect(status().isInternalServerError())
                .andExpect(content().string("No ratings found for product"));
    }
    
    @Test
    void testGetRatingsByCustomerId_NoRatingsFound() throws Exception {
        // Arrange
        Integer customerId = 1;
        when(ratingService.getRatingsByCustomerId(customerId)).thenThrow(new RatingException("No ratings found for customer"));
        
        // Act & Assert
        mockMvc.perform(get("/rating/customer/{customerId}", customerId))
                .andExpect(status().isInternalServerError())
                .andExpect(content().string("No ratings found for customer"));
    }
    
    @Test
    void testDeleteRating_RatingNotFoundException() throws Exception {
        // Arrange
        Integer ratingId = 1;
        
        // Mock the service to throw an exception when trying to delete
        doThrow(new RatingException("Rating not found")).when(ratingService).deleteRating(ratingId);
